...
export function drawFinder(
  ...
  const innerTotalSize =
    (size - innerSizeModifier) * unit + diameter - padding * 2; // Add the diameter manually as stroke is not used here
  const innerSideSize = innerTotalSize - rectCornerSize * 2;
  const innerPositionX =
    unit * (x + innerSizeModifier / 2) + padding + diameter; // Move right to exclude the top-left corner
  ...
  return `
    <path d="${outer.join(
      " "
    )}" fill="none" stroke="currentColor" stroke-width="${diameter}" />
    <path d="${inner.join(" ")}" fill="currentColor" stroke="none" />
  `;
}

export function drawAlignment(x: number, y: number, size: number = 5): string {
  const outerTotalSize = size * unit - padding * 2;
  const outerSideSize = outerTotalSize - rectCornerSize * 2;
  const outerPositionX = unit * x + padding + radius + diameter; // Move right to exclude the top-left corner
  const outerPositionY = unit * y + padding + radius;
